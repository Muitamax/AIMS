...
require_once("../email_config.php"); // Include email functions
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($_POST['std'])) {
            ...
            $st_password = password_hash($st_id, PASSWORD_BCRYPT); // Default password is student ID
            
            $stmt = $conn->prepare("INSERT INTO students (st_id, st_name, st_dept, st_batch, st_sem, st_email, st_password) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssiiss", $st_id, $st_name, $st_dept, $st_batch, $st_sem, $st_email, $st_password);
            
            if ($stmt->execute()) {
                $success_msg = "✅ Student added successfully!";
                // Send email notification
                sendAccountCreationNotification($st_email, $st_name, $st_id, 'student');
            } else {
        ...
        if (isset($_POST['tcr'])) {
            ...
            $tc_password = password_hash($tc_id, PASSWORD_BCRYPT); // Default password is teacher ID
            
            $stmt = $conn->prepare("INSERT INTO teachers (tc_id, tc_name, tc_dept, tc_email, tc_course, tc_password) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $tc_id, $tc_name, $tc_dept, $tc_email, $tc_course, $tc_password);
            
            if ($stmt->execute()) {
                $success_msg = "✅ Teacher added successfully!";
                // Send email notification
                sendAccountCreationNotification($tc_email, $tc_name, $tc_id, 'teacher');
            } else {
